...
export const getQueryFn: <T>(options: {
  ...
  async ({ queryKey }) => {
    ...
    // Log token state for debugging auth endpoints
    if (url.includes('/api/auth/')) {
      console.log(`Auth request to ${url}`, {
        tokenExists: accessToken ? true : false,
        tokenPreview: accessToken ? `${accessToken.slice(0, 10)}...${accessToken.slice(-5)}` : 'none'
      });
    }
    ...
    try {
      ...
      console.log(`Making request to ${cacheBustUrl} with headers:`, {
        authorization: headers['Authorization'] ? 'Bearer token included' : 'No token',
        cacheControl: headers['Cache-Control'] || 'default'
      });
      
      // Make the request with properly constructed headers
      // Use the getApiUrl function to get the full API URL
      const fullUrl = getApiUrl(cacheBustUrl);
      console.log(`Making request to ${fullUrl}`);
