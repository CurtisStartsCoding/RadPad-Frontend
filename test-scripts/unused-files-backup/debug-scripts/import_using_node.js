/**
 * Script to import the SQL file using Node.js directly
 * This avoids the psql authentication issues
 */
...
async function importUsingNode() {
  let client;
  ...
  try {
    ...
    // Extract and import the mappings and markdown docs
    console.log('\n=== EXTRACTING AND IMPORTING MAPPINGS AND MARKDOWN DOCS ===');
    ...
    console.log('Reading source file and importing statements...');
    
    // Start a transaction for the import
    await client.query('BEGIN');
    
    for await (const line of lineReader) {
      // Check if this is the start of a mapping or markdown doc statement
      if (line.includes('INSERT INTO medical_cpt_icd10_mappings') || 
          line.includes('INSERT INTO medical_icd10_markdown_docs')) {
        
        // If we were already in a multiline statement, execute it first
        if (inMultilineStatement && currentStatement.trim() !== '') {
          try {
            await client.query(currentStatement);
            
            if (currentStatement.includes('medical_cpt_icd10_mappings')) {
              mappingCount++;
              if (mappingCount % 100 === 0) {
                console.log(`Imported ${mappingCount} mapping statements`);
              }
            } else if (currentStatement.includes('medical_icd10_markdown_docs')) {
              docCount++;
              if (docCount % 100 === 0) {
                console.log(`Imported ${docCount} markdown doc statements`);
              }
            }
          } catch (error) {
            console.error(`Error executing statement: ${error.message}`);
            console.error(`Problematic statement: ${currentStatement.substring(0, 100)}...`);
        ...
        // If the statement ends on this line, execute it immediately
        if (line.endsWith(';')) {
          try {
            await client.query(currentStatement);
      ...
      // If we're in a multiline statement, append this line
      else if (inMultilineStatement) {
        currentStatement += '\n' + line;
        
        // If the statement ends on this line, execute it
        if (line.endsWith(';')) {
          try {
            await client.query(currentStatement);
    ...
    // If we're still in a multiline statement at the end of the file, execute it
    if (inMultilineStatement && currentStatement.trim() !== '') {
      try {
        await client.query(currentStatement);
